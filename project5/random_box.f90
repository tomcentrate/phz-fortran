...
! Also compute entropy vs. time
	...
	INTEGER, PARAMETER :: ntmax=10000 ! maximum number of time steps
	INTEGER, PARAMETER :: nemax=100000 ! number of walks in ensemble
	INTEGER, PARAMETER :: ient=10 ! Number of steps to update p array
	INTEGER, PARAMETER :: ipmax=1000 ! Maximum number of time elements in p
	...
	REAL(Kind=Prec14) :: t
	open(unit =10, file="entropy.out")  ! Write entropy to file
	open(unit =11, file="positions.out") ! Write positiosn to file
	! set r2a=0, p=0
	r2a=0.0d0
	p=0.0d0
	entropy= 0.0d0
	...
	do ne=1,nemax ! Nemax realizations of random walk
		...
		!Calculate the probability for every ient set
	...
	do nt=1, ntmax! Loop for r^2
	 write(11,100) nt, r2a(nt)/dfloat(nemax)
	enddo ! end position write
	! Calculate entropy loop
	do it=1, ipmax
	entropy = 0
	 do ix=1,nxg ! loop through 
	   do iy=1,nyg
		if(p(ix,iy,it).GT.0) then  ! Avoid calculation if probability is zero
	 	entropy=entropy-(p(ix,iy,it)/dfloat(nemax))*dlog(p(ix,iy,it)/dfloat(nemax))
		endif
	   enddo ! end iy loop
	 enddo ! end ix loop
	  write(10,100) it,entropy
	enddo ! end time loop
